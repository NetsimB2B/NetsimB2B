    ...
    <div className="page order-detail-page">
      ...
      <div className="order-detail-layout">
        ...
        <aside className="order-detail-sidebar">
          <Card className="order-status-card"><div className="order-status-icon">{order.status === "Sevk Edildi" ? "✓" : "◷"}</div><h2>{statusDescriptions[order.status]}</h2><p>Siparişinizin güncel operasyon durumu aşağıda yer almaktadır.</p><div><span>Güncel durum</span><Badge tone={statusTone(order.status)}>{order.status}</Badge></div><div><span>Planlanan teslimat</span><strong>{order.expectedDeliveryDate ? formatDate(order.expectedDeliveryDate) : "Planlanıyor"}</strong></div>{order.shipmentId && <Link className="button button-primary" to={`/sevkiyatlar/${order.shipmentId}`}>Sevkiyatı Görüntüle</Link>}</Card>
          <Card className="order-documents-card"><h3>Sipariş Belgeleri</h3><Link to={order.invoiceId ? `/faturalar/${order.invoiceId}` : "/faturalar"}><span>▤</span><p><strong>Fatura</strong><small>{order.invoiceId ?? "Henüz oluşmadı"}</small></p><b>›</b></Link><Link to={order.shipmentId ? `/sevkiyatlar/${order.shipmentId}` : "/sevkiyatlar"}><span>▣</span><p><strong>İrsaliye / Sevkiyat</strong><small>{order.shipmentId ?? "Hazırlanıyor"}</small></p><b>›</b></Link>{order.quoteId && <Link to={`/teklifler/${order.quoteId}`}><span>◇</span><p><strong>Teklif</strong><small>{order.quoteId}</small></p><b>›</b></Link>}</Card>
          <Card className="order-contact-card"><span>SY</span><div><small>Müşteri temsilciniz</small><strong>{order.salesRepresentative ?? "Netsim Satış"}</strong><Link to={`/destek?type=Sipariş&ref=${order.id}`}>Sipariş desteği al →</Link></div></Card>
        </aside>
      </div>
    </div>
  );
